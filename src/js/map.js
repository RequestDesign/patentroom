...
$(document).ready(function () {
  ...
  function init() {
    // Создаём карту с центром на заданных координатах
    const map = new ymaps.Map("map", {
      center: [55.7158, 37.7911], // Москва, Рязанский проспект 75
      zoom: 15,
  ...
  `;
    ...
    // Применяем CSS-фильтр для серого стиля карты
    map.panes.get("ground").getElement().style.filter = "grayscale(100%)";

    // Создание пользовательского маркера с картинкой
    const customPlacemark = new ymaps.Placemark(
      [55.7158, 37.7911], // Координаты маркера
      {
        balloonContent:
      ...
      },
      {
        ...
        iconImageSize: [0, 0], // Размер маркера по умолчанию
      }
    );
